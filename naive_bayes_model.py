...
# 5. Scale features (important for Naive Bayes)
...
# 8. Feature importance (using class probabilities)
...
joblib.dump(nb_model, "naive_bayes_subscription_model.joblib")
joblib.dump(scaler, "naive_bayes_scaler.joblib")
print(f"\nModel saved as 'naive_bayes_subscription_model.joblib'")
print(f"Scaler saved as 'naive_bayes_scaler.joblib'")

# 10. Example prediction function
def predict_subscription(service_usage_count, avg_interval, total_spend, account_age_days, 
                        help_desk_tickets, last_login_days_ago, feature_usage_count, 
                        peak_usage_hour, mobile_usage_ratio):
    ...
    probability = nb_model.predict_proba(new_data_scaled)[0]
...
# Example usage
example_result = predict_subscription(25, 15, 2500, 365, 3, 5, 12, 14, 0.7)
